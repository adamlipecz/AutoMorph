...
def process(image_list, save_path):
    ...
    for image_path in image_list:
        ...
        if os.path.exists(f'{AUTOMORPH_DATA}/Results/M0/images/' + image_path):
            print(f'[M0] {image_path} already has been processed, continuing with next image...')
            continue
        try:
    ...
    print(f"[M0] Crop information saved to: {f'{AUTOMORPH_DATA}/Results/M0/crop_info.csv'}")
...
if __name__ == "__main__":
    ...
    # List all of the images in the image_list
    print("[M0] Images found in directory:")
    for img_name in image_list:
        print(img_name)
    
    save_path = f'{AUTOMORPH_DATA}/Results/M0/images/'
    print(f"[M0] Saving preprocessed images to: {save_path}")
    if not os.path.exists(save_path):
        print(f"[M0] Creating directory: {save_path}")
